...
void handleJoin(int sock, int32_t* playerId, char* playerName);
...
void resetGame();
void resetPlayers();
int isGameEnd(); // Atrgriež 1, ja ir 0, ja nav
int isPointsLeft();
int isAnyPacmanLive();
...
void* handleClient(void *sockets) {
    ...
    //------- JOIN / ACK -------//
    handleJoin(sockTCP, &playerId, (char *)&playerName);
