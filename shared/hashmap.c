...
size_t hashmap_hash(const void* key) {
    uint32_t intKey = *(uint32_t*)key;
    intKey = (intKey ^ 61) ^ (intKey >> 16);
    intKey = intKey + (intKey << 3);
    intKey = intKey ^ (intKey >> 4);
    intKey = intKey * 0x27d4eb2d;
    intKey = intKey ^ (intKey >> 15);
    return intKey;
}

/*
 * Def§ult key comparison function
 */
int hashmap_compare_keys(const void* key1, const void* key2) {
    uint32_t intKey1 = *(uint32_t*)key1;
    uint32_t intKey2 = *(uint32_t*)key2;
    if (intKey1 == intKey2) {
        return 0;
    }
    return -1;
